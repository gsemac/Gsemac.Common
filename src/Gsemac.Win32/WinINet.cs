...
namespace Gsemac.Win32 {
    ...
    public static class WinINet {
        ...
        /// </summary>
        public const uint INTERNET_COOKIE_EVALUATE_P3P = 0x80;
        /// <summary>
        ...
        /// <param name="lpszCookieName">Pointer to a null-terminated string that specifies the name to be associated with the cookie data. If this parameter is <see cref="null"/>, no name is associated with the cookie.</param>
        /// <param name="lpszCookieData">Pointer to the actual data to be associated with the URL.</param>
        /// <returns>Returns <see cref="true"/> if successful, or <see cref="false"/> otherwise. To get a specific error message, call <see cref="Marshal.GetLastWin32Error"/>.</returns>
        ...
        /// <param name="lpszUrlName">Pointer to a null-terminated string that specifies the URL for which the cookie should be set.</param>
        /// <param name="lpszCookieName">Pointer to a null-terminated string that specifies the name to be associated with the cookie data. If this parameter is <see cref="null"/>, no name is associated with the cookie.</param>
        /// <param name="lpszCookieData">Pointer to a null-terminated string that contains the data to be associated with the new cookie.<para></para>If this pointer is <see cref="null"/>, <see cref="InternetSetCookieEx"/> fails with an ERROR_INVALID_PARAMETER error.</param>
        /// <param name="dwFlags">Flags that control how the function retrieves cookie data.</param>
        /// <param name="dwReserved"><see cref="null"/>, or contains a pointer to a Platform-for-Privacy-Protection (P3P) header to be associated with the cookie.</param>
        /// <returns>Returns a member of the InternetCookieState enumeration if successful, or <see cref="false"/> if the function fails. On failure, if a call to GetLastError returns ERROR_NOT_ENOUGH_MEMORY, insufficient system memory was available.</returns>
